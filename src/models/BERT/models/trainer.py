...
class Trainer(object):
    ...
    def validate(self, valid_iter, step=0):
        ...
        with torch.no_grad():
            for batch in valid_iter:
                ...
                outputs, _, _,poolings = self.model(src, tgts, segs, clss, mask_src, mask_tgts, mask_cls, self.args.merge, self.args.inter_weight, role_mask)

                batch_stats = self.loss.monolithic_compute_loss(batch, outputs,poolings)
                for i in range(2):
    ...
    def _gradient_accumulation(self, true_batchs, normalization, total_stats,
        ...
        for batch in true_batchs:
            ...
            tgts = [tgt_user, tgt_agent]
            mask_tgts = [mask_tgt_user, mask_tgt_agent]
            outputs, _, scores, poolings = self.model(src, tgts, segs, clss, mask_src, mask_tgts, mask_cls, self.args.merge, self.args.inter_weight, role_mask)
            batch_stats = self.loss.sharded_compute_loss(batch, outputs, poolings, self.args.generator_shard_size, normalization, self.args, scores)
            for i in range(2):
