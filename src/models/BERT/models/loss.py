...
class LossComputeBase(nn.Module):
    ...
    def _compute_loss(self, batch, output,poolings, target, **kwargs):
        """
    ...
    def monolithic_compute_loss(self, batch, output,poolings):
        """
        ...
        _, batch_stats = self._compute_loss(batch, output, poolings, [batch.tgt_user[:, 1:], batch.tgt_agent[:, 1:]])

        return batch_stats

    def sharded_compute_loss(self, batch, output,poolings,
                              shard_size,
                             normalization, args, scores=None):
        ######################################################################################
        ...
        loss, stats = self._compute_loss(batch, output,poolings, [batch.tgt_user[:, 1:], batch.tgt_agent[:, 1:]], scores)
        for i in range(2):
        ...
        total_loss = args.role_weight * loss[0] + (1 - args.role_weight) * loss[1] \
                     + loss[2] * args.topic_weight + (loss[3] + loss[4]) * args.kl_weight
        total_loss.backward()
...
class NMTLossCompute(LossComputeBase):
    ...
    def _compute_loss(self, batch, output,poolings, target, score=None):
        losses, stats = [], []
        for i in range(2):
        ...
        topic_loss = self.BCEloss(poolings[0], poolings[1])
        losses.append(topic_loss)

        if score is not None:

            user_dec_user_att = torch.cat(score[0][0], dim=0).view(-1, score[2].shape[1], score[2].shape[2]) # (12 * bs, user_len, src_len)
            user_dec_agent_att = torch.cat(score[1][0], dim=0).view(-1, score[2].shape[1], score[2].shape[2])  # (12 * bs, user_len, src_len)
            agent_dec_user_att = torch.cat(score[0][1], dim=0).view(-1, score[3].shape[1], score[3].shape[2])  # (12 * bs, user_len, src_len)
            agent_dec_agent_att = torch.cat(score[1][1], dim=0).view(-1, score[3].shape[1], score[3].shape[2])  # (12 * bs, user_len, src_len)
            ...
            user_dec_user_att = F.softmax(user_dec_user_att, dim=-1) * score[2]  # (12 * bs, user_len, src_len)
            user_dec_agent_att = F.softmax(user_dec_agent_att, dim=-1) * score[2]  # (12 * bs, user_len, src_len)
            agent_dec_user_att = F.softmax(agent_dec_user_att, dim=-1) * score[3]  # (12 * bs, user_len, src_len)
            agent_dec_agent_att = F.softmax(agent_dec_agent_att, dim=-1) * score[3]  # (12 * bs, user_len, src_len)
