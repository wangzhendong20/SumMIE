...
from models.encoder import TransformerEncoder,AttentivePooling
...
def build_optim_dec(args, model, checkpoint):
    ...
    if args.fusion_flag:
        for n, p in params:
            if n.endswith('p_init') or n.endswith('p_aver_mul'):
...
class AbsSummarizer(nn.Module):
    def __init__(self, args, device, checkpoint=None, bert_from_extractive=None):
        ...
        self.device = device
        self.bert = Bert(args.large, args.temp_dir, args.finetune_bert)
        self.encoder = TransformerEncoder(self.bert.model.config.hidden_size, args.ext_ff_size, args.ext_heads,
                                               args.ext_dropout, 6)
        self.attentivePooling = AttentivePooling(self.bert.model.config.hidden_size,self.bert.model.config.hidden_size)
    ...
    def forward(self, src, tgts, segs, clss, mask_src, mask_tgts, mask_cls, merge_type, inter_weight, role_mask):
        top_vec = self.bert(src, segs, mask_src)

        user_mask = mask_src & role_mask.ne(1)
        agent_mask = mask_src & role_mask.ne(2)
        top_vec_user = self.encoder(top_vec,user_mask)
        top_vec_agent = self.encoder(top_vec,agent_mask)

        top_vec_user_pool = self.attentivePooling(top_vec_user)
        top_vec_agent_pool = self.attentivePooling(top_vec_agent)
        ...
        return [user_outputs, agent_outputs], None, [user_scores, agent_scores, kl_mask_user, kl_mask_agent],[top_vec_user_pool,top_vec_agent_pool]
