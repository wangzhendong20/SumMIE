...
class TransformerDecoderLayer(nn.Module):
    ...
    def __init__(self, d_model, heads, d_ff, dropout):
        ...
        self.heads = heads
        self.dim_per_head = d_model // heads
        ...
        self.p_init = nn.Parameter(torch.tensor(3.0), requires_grad=False)
        self.sig = nn.Sigmoid()

        self.p_aver_mul = nn.Parameter(torch.tensor(3.0), requires_grad=False)
    ...
    def forward(self, inputs, memory_bank, inter_memory_bank, src_pad_mask, tgt_pad_mask, inter_pad_mask,
        ...
        user_attn = F.softmax(user_score,dim=-1)
        agent_attn = F.softmax(agent_score,dim=-1)

        if inter_memory_bank is not None:

            query_cross, role_scores = self.cross_attn(inter_memory_bank, inter_memory_bank, query_norm,
                                             mask=inter_dec_mask,
                                             layer_cache=inter_layer_cache,
                                             type="role")


            role_attn = F.softmax(role_scores,dim=-1)
            pad_mount = user_attn.shape[-1] - role_attn.shape[-1]
            role_attn = F.pad(role_attn,(0,pad_mount))


            p_av = self.p_init
            p_alpha_aver = self.sig(p_av)

            fused_attn_ur_1 = p_alpha_aver * user_attn + (1 - p_alpha_aver) * role_attn
            fused_attn_ar_1 = p_alpha_aver * agent_attn + (1 - p_alpha_aver) * role_attn

            fused_attn_ur_2 = user_attn * role_attn
            fused_attn_ar_2 = agent_attn * role_attn

            p_av_mul = self.p_aver_mul
            p_alpha_aver_mul = self.sig(p_av_mul)

            fused_attn_ur = p_alpha_aver_mul * fused_attn_ur_1 + (1-p_alpha_aver_mul) * fused_attn_ur_2
            fused_attn_ar = p_alpha_aver_mul * fused_attn_ar_1 + (1-p_alpha_aver_mul) * fused_attn_ar_2


            values = self.linear_values(memory_bank)

            values = values.view(memory_bank.shape[0], -1, self.heads, self.dim_per_head) \
                .transpose(1, 2)

            mid_fused_user = torch.matmul(fused_attn_ur,values).transpose(1, 2).contiguous() \
                .view(memory_bank.shape[0], -1, self.heads * self.dim_per_head)
            mid_fused_agent = torch.matmul(fused_attn_ar,values).transpose(1, 2).contiguous() \
                .view(memory_bank.shape[0], -1, self.heads * self.dim_per_head)
            mid_fused_user = self.final_linear(mid_fused_user)
            mid_fused_agent = self.final_linear(mid_fused_agent)
            ...
            if merge_type == 'tanh':
                ...
                cat_query = torch.cat([mid_fused_user, mid_fused_agent, query_cross], dim=-1).view(-1, mid_user.shape[-1] * 3)
