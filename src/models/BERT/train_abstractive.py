    ...
    if (args.test_all):
        ...
        for i, cp in enumerate(cp_files):
            ...
            if (args.test_start_from != -1 and step < args.test_start_from):
                xent_lst.append(([1e6, 1e6, 1e6, 1e6], cp))
                continue
            ...
            # if (i - max_step > 10):
            #     break
        xent_lst = sorted(xent_lst, key=lambda x: x[0][-1])[:5]
...
def validate(args, device_id, pt, step):
    ...
    tokenizer = BertTokenizer.from_pretrained('../pretrained/bert_base_chinese', do_lower_case=True, cache_dir=args.temp_dir)
    symbols = {'BOS': tokenizer.vocab['[unused99]'], 'EOS': tokenizer.vocab['[unused1]'],
               'PAD': tokenizer.vocab['[PAD]'], 'EOQ': tokenizer.vocab['[unused2]']}

    valid_loss = abs_loss(model.generator, symbols, model.vocab_size, train=False, device=device)

    trainer = build_trainer(args, device_id, model, None, valid_loss, tokenizer)
    stats = trainer.validate(valid_iter, step)
    # role weight
    return [stats[0].xent(), stats[1].xent(), 0.2 * stats[0].xent() + 0.8 * stats[1].xent()]
...
def test_text_abs(args, device_id, pt, step):
    ...
    test_iter = data_loader.Dataloader(args, load_dataset(args, 'test', shuffle=False),
                                       args.test_batch_size, device,
                                       shuffle=False, is_test=True)
    tokenizer = BertTokenizer.from_pretrained('bert_base_chinese', do_lower_case=True, cache_dir=args.temp_dir)
    symbols = {'BOS': tokenizer.vocab['[unused99]'], 'EOS': tokenizer.vocab['[unused1]'],
               'PAD': tokenizer.vocab['[PAD]'], 'EOQ': tokenizer.vocab['[unused2]']}
    predictor = build_predictor(args, tokenizer, symbols, model, logger)
    predictor.translate(test_iter, step)
...
def train_abs_single(args, device_id):
    ...
    trainer = build_trainer(args, device_id, model, optim, train_loss, tokenizer)
